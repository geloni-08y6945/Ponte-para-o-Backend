...
async function buscarPrevisaoDetalhada(cidade) {
    ...
    // ATENÇÃO: Verifique se a porta (3001) é a mesma do seu server.js
    // Em um ambiente de produção, esta URL seria a do seu backend publicado (ex: no Render).
    const backendUrl = `http://localhost:3001/api/previsao/${encodedCidade}`;

    console.log(`[Frontend] Solicitando previsão para: ${cidade} ao backend: ${backendUrl}`); // Para debug
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (errorData && errorData.error) {
                    errorMsg = errorData.error; // Mensagem de erro vinda do backend
                }
            } catch (e) {
                // Falha ao parsear JSON de erro, ou o backend não enviou um JSON de erro estruturado
        ...
        console.log('[Frontend] Dados da previsão recebidos do backend:', data); // Para debug
        return data; // Os dados devem estar no mesmo formato que a OpenWeatherMap enviava
    } catch (error) {
        // O erro já pode ter sido construído com a mensagem do backend
        // ou pode ser um erro de rede (fetch falhou completamente)
        console.error('[Frontend] Erro em buscarPrevisaoDetalhada (via backend):', error.message);
        // Relança o erro para ser pego pelo bloco catch no event listener
        throw error;
    }
}
 ...
 * (ESTA FUNÇÃO NÃO PRECISA MUDAR, POIS O FORMATO DOS DADOS DO BACKEND DEVE SER O MESMO DA OPENWEATHERMAP)
 * @param {Object} data - Os dados completos retornados pela API OpenWeatherMap Forecast (via backend).
...
function exibirPrevisaoDetalhada(previsaoDiariaProcessada, nomeCidade) {
    ...
    previsaoDiariaProcessada.forEach(dia => {
        ...
        // Adicionar 'T00:00:00' para ajudar a tratar como data local meia-noite,
        // e timeZone: 'UTC' para consistência na formatação, evitando problemas com fuso horário do browser
...
// O RESTANTE DO SEU CÓDIGO js/principal.js PERMANECE O MESMO ABAIXO DESTA LINHA
// ======================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Cache.informacoesVeiculoDiv = document.getElementById('informacoesVeiculo'); // Já estava aqui, cuidado com duplicação se copiou e colou
    ...
    if (verificarClimaBtn && cidadeInput && previsaoResultadoDiv) {
        verificarClimaBtn.addEventListener('click', async () => {
            ...
            try {
                // buscarPrevisaoDetalhada agora chama seu backend!
...
// O restante das suas funções (buscarDetalhesVeiculoAPI, showNotification, etc.) continua aqui...
// ...
// COLOQUE O RESTANTE DO SEU CÓDIGO `principal.js` AQUI
// ... ( IDs para mapeamento com dados_veiculos_api.json, buscarDetalhesVeiculoAPI, etc. ) ...
// ... ( todas as suas outras funções de Veiculo, Garagem, UI, etc. ) ...
...
async function buscarDetalhesVeiculoAPI(identificadorVeiculo) {
    ...
    try {
    const response = await fetch('./dados_veiculos_api.json'); // Assegure que o caminho está correto
    if (!response.ok) {
    ...
    await new Promise(resolve => setTimeout(resolve, 600 + Math.random() * 400)); // Simula delay
...
function showNotification(message, type = 'info', duration) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            notification.style.transform = 'translateX(110%)'; // Ajuste para sair corretamente
            setTimeout(() => notification.remove(), 500); // Tempo para a animação de saída
        }, finalDuration);
...
function verificarLembretesManutencao(veiculo) {
    ...
    try {
        ...
        veiculo.historicoManutencao.filter(m => m instanceof Manutencao && m.isAgendamento()).forEach(m => {
            const dataM = m.getDataObj(); // Já é um objeto Date
...
function prefillModifyForm(veiculo) {
    ...
    setVal(Cache.nicknameInput, veiculo.nickname); setVal(Cache.imagemInput, ''); // Limpa o campo de imagem
...
async function handleVehicleAction(event) {
    ...
    try {
        if (acao === 'verDetalhesAPI') {
            ...
            updateDisplayContent(v); // Garante que a UI é atualizada após a ação
        } else {
...
function handleMaintenanceSubmit(event) {
    ...
    try { 
        ...
        v.adicionarManutencao(new Manutencao(data, tipo, custo, desc));
        ...
        updateManutencaoUI(v); // Atualiza a UI após adicionar
    } catch (error) { showNotification(`Erro manutenção: ${error.message}`, 'error'); console.error("Erro submit manut:", error); }
}
// A função DOMContentLoaded já está definida no topo com as modificações da previsão.
